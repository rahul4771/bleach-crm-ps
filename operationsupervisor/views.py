...
from bleach_crm_ps.permissions import IsOperationSupervisor
...
class OrderDetails(IsOperationSupervisor,View):
	def get(self,request):
		...
		filters=[] 
		if fil_status:
			if fil_status == 'ORDER_IN_PROGRESS' or fil_status == 'ORDER_CLOSED' or fil_status == 'ORDER_CANCELLED' or fil_status == 'APPROVED-NOT PAID' or fil_status == 'EVALUATING':
...
class ClientOrderDetails(IsOperationSupervisor,View):
	def get(self,request,order_id):
		...
		return render(request,"operationsupervisor/client/order-page.html",{"order":order,"client_details":client_details,"active_orders_count":active_orders_count,"total_orders_count":total_orders_count,"average_feedback":average_feedback,})


class ClientOrders(IsOperationSupervisor,View):
	def get(self,request,client_id):
		...
		return render(request,"operationsupervisor/client/client-page.html",{"client_details":client_details,"orders":orders,"active_orders_count":active_orders_count,})


class OperationSupervisorHome(IsOperationSupervisor,View):
	def get(self,request):
	...
	def post(self,request):
		...
		if action == 'edit_cleaning':
			...
			if schedule_id and not check_cleaners_assigned and not check_tl_assigned and start_at == ((order_schedule.start_at+timedelta(hours=3)).replace(tzinfo=None)) and end_at == ((order_schedule.end_at+timedelta(hours=3)).replace(tzinfo=None)) and cleaningteamstatus == 'ASSIGNED':
				...
				cleaning_team                = CleaningTeam.objects.get(is_active=True,order_scheduler_id=schedule_id,team_leader__user_type='TEAMINCHARGE')
		...
		if action == 'edit_followup':
			...
			cleaning_time   = request.POST.get('followup_time')
			cleaning_hours 	= float(request.POST.get('followup_hours'))
			start_at        = datetime.strptime(cleaning_date+' '+cleaning_time,'%d-%m-%Y %I:%M %p')
			end_at          = start_at + timedelta(hours=cleaning_hours)
			...
			if schedule_id and not check_cleaners_assigned and not check_tl_assigned and start_at == ((followup_schedule.start_at+timedelta(hours=3)).replace(tzinfo=None)) and end_at == ((followup_schedule.end_at+timedelta(hours=3)).replace(tzinfo=None)) and followupteamstatus == 'ASSIGNED':
				...
				followup_team                = FollowUpTeam.objects.get(is_active=True,followup_scheduler_id=schedule_id,team_leader__user_type='TEAMINCHARGE')
...
class TicketDetails(IsOperationSupervisor,View):
	def get(self,request):
		...
		return render(request,'operationsupervisor/ticket/tickets.html',{"tickets":tickets,"follow_ups_count":follow_ups_count,"follow_up_cleaning_count":follow_up_cleaning_count,"search_query":search,"page_range":page_range,"entry_per_page":entry_per_page,"no_of_entries":no_of_entries,"governorates":governorates,"areas":areas,"investigators":investigators,"fil_governorate":fil_governorate,'fil_area':fil_area,"fil_investigator":fil_investigator,"fil_status":fil_status,})		

class TicketAdvanced(IsOperationSupervisor,View):
	def get(self,request,client_id,followup_id):
		...
		return render(request,'operationsupervisor/ticket/followup-page.html',{"client_details":client_details,"active_orders_count":active_orders_count,"total_orders_count":total_orders_count,"followup_details":followup_details,})		

class ResourceManagement(IsOperationSupervisor,View):
	def get(self,request):
...
class AssigncleaningTeam(IsOperationSupervisor,View):
	def get(self,request,scheduler_id):
		...
		return render(request,'operationsupervisor/cleaning/cleaningteam_assign.html',{'order_schedule':order_schedule,'cleaners':cleaners,'leaders':leaders,'drivers':drivers,})
	...
	def post(self,request,scheduler_id):
		...
		if	cleaning_team_assign_form.is_valid() and not check_cleaners_assigned and not check_tl_assigned:
			...
			return render(request,'operationsupervisor/cleaning/cleaningteam_assign.html',{'cleaning_team_assign_form':cleaning_team_assign_form,'order_schedule':order_schedule,'cleaners':cleaners,'leaders':leaders,'drivers':drivers})	
...
class AssignFollowupTeam(IsOperationSupervisor,View):
	def get(self,request,scheduler_id):
		...
		return render(request,'operationsupervisor/cleaning/followupteam_assign.html',{'follow_up_team_assign_form':follow_up_team_assign_form,'followup_schedule':followup_schedule,'cleaners':cleaners,'leaders':leaders,'drivers':drivers,"sections":followupsections,"keynotes":followupkeynotes})
	...
	def post(self,request,scheduler_id):
		...
		if	follow_up_team_assign_form.is_valid() and not check_cleaners_assigned and not check_tl_assigned:
			...
			return render(request,'operationsupervisor/cleaning/followupteam_assign.html',{'follow_up_team_assign_form':follow_up_team_assign_form,'followup_schedule':followup_schedule,'cleaners':cleaners,'leaders':leaders,'drivers':drivers,})	
...
class InvestigationTask(IsOperationSupervisor,View):
	...
	def post(self,request,investigation_id):
		...
		if form_action == "followup":
			return redirect('op-supervisor:follow-up', investigation_id)
		if form_action == "discount":
			return redirect('op-supervisor:cash-back', investigation_id)
		if form_action == "gift":
...
class Followup(IsOperationSupervisor,View):
...
class FollowupEdit(IsOperationSupervisor,View):
	service_formset_define    = formset_factory(QuatationServiceForm)
	def get(self,request,investigation_id):
		investigation = Investigation.objects.get(is_active=True,id=int(investigation_id))
		print(investigation.order_schedule.evaluation_details.id,"evs")
...
class FollowupDelete(IsOperationSupervisor,View):
	def get(self,request,investigation_id):
...
class Cashback(IsOperationSupervisor,View):
	def get(self,request,investigation_id):
		# followup status change
		follow_up = FollowUp.objects.get(investigation_id=investigation_id,is_active=True)
		follow_up.status = 'FOLLOWUP_IN_PROGRESS'
...
class CashbackEdit(IsOperationSupervisor,View):
	def get(self,request,investigation_id):
...
class CashbackDelete(IsOperationSupervisor,View):
	def get(self,request,investigation_id):
...
class InternalReport(IsOperationSupervisor,View):
	def get(self,request,investigation_id):
		# followup status change
		follow_up = FollowUp.objects.get(investigation_id=investigation_id,is_active=True)
		follow_up.status = 'FOLLOWUP_IN_PROGRESS'
...
class InternalReportEdit(IsOperationSupervisor,View):
	def get(self,request,investigation_id):
...
class InternalReportDelete(IsOperationSupervisor,View):
	def get(self,request,investigation_id):
...
class BuyBackPromoCode(IsOperationSupervisor,View):
	def get(self,request,investigation_id):
		# followup status change
		follow_up = FollowUp.objects.get(investigation_id=investigation_id,is_active=True)
		follow_up.status = 'FOLLOWUP_IN_PROGRESS'
...
class BuyBackPromoCodeEdit(IsOperationSupervisor,View):
	def get(self,request,investigation_id):
...
class BuyBackPromoCodeDelete(IsOperationSupervisor,View):
	def get(self,request,investigation_id):
