...
class StlHome(IsSeniorTeamLeader,View):
	def get(self,request):
		...
		datechangeschedules = OrderScheduler.objects.select_related('order__evaluation','order_scheduler_book').filter(order__evaluation__evaluation_id='BLC20210310115')
		for datechangeschedule in datechangeschedules:
			datechangeschedule.start_at       = datechangeschedule.start_at-timedelta(hours=1)
			datechangeschedule.no_of_cleaners = datechangeschedule.order_scheduler_book.number_of_cleaners
			datechangeschedule.cleaning_hours = 2
			datechangeschedule.save()
