...
class StlHome(IsSeniorTeamLeader,View):
	def get(self,request):
		...
		calendar_order_schedules_alls       = OrderScheduler.objects.filter(Q(~Q(work_status='CLEANING_CANCELLED')&~Q(work_status='CLEANING_FULFILLED'))).filter(status='CONFIRMED',is_active=True,start_at__lt=teamassign_to_date+timedelta(14)).select_related('order__evaluation__customer','customer_address','order_scheduler_book').filter(order__evaluation__quatation_status='APPROVED',order__is_advance=False).filter(Q( Q(Q(order__payment_status='COMPLETED')|~Q(order__preamount_paid = 0)) | Q(order__evaluation__payment_method='POSTPAID') | Q(order__evaluation__payment_method='SUBSCRIPTION') )).annotate(duplicate=Concat('start_at','end_at','order__id',output_field=CharField()),cleaning_team_count=Count('cleaning_team_order_scheduler'))
		...
		#cleaning team assignment task
		try:
			assign_order_schedules     = OrderScheduler.objects.filter(id__in=calendar_order_schedules_list).select_related('evaluation_details__evaluator','evaluation_details__evaluation','order_scheduler_book').prefetch_related('evaluation_details__evaluation__booking_evaluation','cleaning_team_order_scheduler__team_leader').annotate(cleaning_team_count=Count('cleaning_team_order_scheduler'))
		except:
	...
	def get(self,request):
		...
		for cid in cleaning_ids:
			...
			if cleaning_data.work_status != 'CLEANING_CANCELLED':
				...
				cleaning_list[3] = cleaning_list[3]+timedelta(hours=3)
				cleaning_list[3] = str(cleaning_list[3].time()) #str(cleaning_data.start_at.time())
				cleaning_list[5] = '-'
				cleaning_list[6] = '-'
