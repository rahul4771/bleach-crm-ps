...
class AdminHome(IsSalesAdmin,View):
	def get(self,request):
		...
		approve_tickets = Investigation.objects.filter(Q(Q(is_paybackdiscount_approved=False)|Q(is_buybackgiftpromo_approved=False))).prefetch_related(Prefetch('followup_investigation',queryset=FollowUp.objects.filter(is_active=True,status='FOLLOWUP_IN_PROGRESS'),to_attr='followup'),Prefetch('paybackdiscount_investigation',queryset=PaybackDiscount.objects.select_related('investigation').filter(is_active=True,investigation__is_paybackdiscount_approved=False),to_attr='paybackdiscounts'),Prefetch('buybackpromocodegift_investigation',queryset=BuybackPromocodeGift.objects.select_related('investigation').filter(investigation__is_buybackgiftpromo_approved=False,is_active=True),to_attr='buybackpromocodegifts'))
		#add days left
		if approve_tickets:
...
class MakeQuatationPhase2Edit(IsSalesAdmin,View):
...
class AddNewService(IsSalesAdmin,View):
	service_formset_define    = formset_factory(QuatationServiceForm)
	def get(self,request,evaluation_detail_id,edit_type):
		...
		return render(request,'salesadmin/enquiry/addservice.html',{'service_formset':self.service_formset_define(),'evaluation_details':evaluation_details,'service_types':service_types,'area_types':area_types,'cleaning_sections':cleaning_sections})	
	...
	def post(self,request,evaluation_detail_id,edit_type):
		...
		if service_formset.is_valid() :
			...
			return render(request,'salesadmin/enquiry/addservice.html',{'service_formset':service_formset,'evaluation_details':evaluation_details,'area_types':area_types,'service_types':service_types,})		

		if edit_type == 'duplicate':
			return redirect('agent:agent-makequatation2duplicateedit',evaluation_details.id)
		else:
			return redirect('bleach_salesadmin:salesadmin-makequatation2edit',evaluation_details.id)