...
class AvailabilityCleaningCallendar(APIView):
	...
	def post(self,request):
		...
		available_cleaners = total_cleaners.exclude(id__in=team_members_scheduled).filter(id__in=shift_cleaners)
		available_leaders  = total_leaders.exclude(id__in=team_leaders_scheduled).filter(id__in=shift_leaders)
	...
	def post(self,request):
		...
		if action == 'edit_cleaning_withautofix':
			for cleaning_schedule in cleaning_schedules:
				...
				active_cleaners1 	= CleaningTeamMember.objects.filter(Q(Q(Q(start_at__gte=schedule_start_at)&Q(start_at__lte=schedule__at))|Q(Q(end_at__gte=schedule_start_at)&Q(end_at__lte=schedule__at))|Q(Q(start_at__lte=schedule_start_at)&Q(end_at__gte=schedule_start_at)&Q(start_at__lte=schedule__at)&Q(end_at__gte=schedule__at))|Q(Q(start_at__gte=schedule_start_at)&Q(end_at__gte=schedule_start_at)&Q(start_at__lte=schedule__at)&Q(end_at__lte=schedule__at)))).exclude(id__in=sameblc_cleaners).values_list("member",flat=True)
				active_cleaners2 	= FollowUpTeamMember.objects.filter(Q(Q(Q(start_at__gte=schedule_start_at)&Q(start_at__lte=schedule__at))|Q(Q(end_at__gte=schedule_start_at)&Q(end_at__lte=schedule__at))|Q(Q(start_at__lte=schedule_start_at)&Q(end_at__gte=schedule_start_at)&Q(start_at__lte=schedule__at)&Q(end_at__gte=schedule__at))|Q(Q(start_at__gte=schedule_start_at)&Q(end_at__gte=schedule_start_at)&Q(start_at__lte=schedule__at)&Q(end_at__lte=schedule__at)))).values_list("member",flat=True)
