...
def CleaningExistingDates(request):
	...
	for team_leader in team_leaders_scheduled_dates:
		index = datetime.strftime(datetime.strptime(team_leader['start_at'].date(),'%Y-%m-%d'),'X%d-X%m-%Y').replace('X0','X').replace('X','')
	...
	for team_member in team_members_scheduled_dates:
		index = datetime.strftime(datetime.strptime(team_member['start_at'].date(),'%Y-%m-%d'),'X%d-X%m-%Y').replace('X0','X').replace('X','')
