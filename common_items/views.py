...
class NewRaiseTicket(IsAuthenticated,View):
	def get(self,request):
		...
		return render(request,'common/ticket/raiseticket.html',{"orders":orders})
...
class ClientOrderDetails(IsAuthenticated,View):
	def get(self,request,order_id):
		...
		visits = Order.objects.select_related('evaluation__customer','evaluation__call_attender').prefetch_related(
...
class AssigncleaningTeam(IsAuthenticated,View):
	def get(self,request,scheduler_id):
		...
		try:
			last_cleaning_team  = CleaningTeam.objects.select_related('order_scheduler__order','team_leader').filter(order_scheduler__order=order_schedule.order,end_at__lte=end_at_datetime).order_by('start_at').prefetch_related(Prefetch('cleaning_member_team',queryset=CleaningTeamMember.objects.select_related('member').filter(is_active=True),to_attr='cleaningteam_members')).last()
		except:
