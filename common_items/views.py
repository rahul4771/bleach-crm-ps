...
class ResourceManagement(IsAuthenticated,View):
	def get(self,request):
		...
		if starting_datetime and ending_datetime:
			workers            =  UserProfile.objects.filter(is_active=True).filter(Q(Q(user_type='TEAMINCHARGE')|Q(user_type='CLEANER'))).prefetch_related('leave_staff').annotate(leave=Sum( Case( When( leave_staff__leave_date=workers_date,then=1),default=0,output_field=IntegerField())) ).prefetch_related(Prefetch('cleaning_member_user',queryset=CleaningTeamMember.objects.filter( Q( Q(is_active=True)&Q(Q(start_at__date__gte=workers_date_start)&Q(end_at__date__lte=workers_date_end)) )).select_related('team__order_scheduler__customer_address__area','team__order_scheduler__order__evaluation','team__order_scheduler__order_scheduler_book'),to_attr='cleaning_member_details'),Prefetch('followup_member',queryset=FollowUpTeamMember.objects.filter(Q( Q(is_active=True)&Q(Q(start_at__date__gte=workers_date_start)|Q(end_at__date__lte=workers_date_end)) )).select_related('team__followup_scheduler__customer_address__area'),to_attr='followup_member_details')).annotate(cleaning_contained=Sum(Case(When(Q(Q(Q(cleaning_member_user__start_at__gte=starting_datetime)&Q(cleaning_member_user__end_at__lte=ending_datetime))|Q(Q(cleaning_member_user__start_at__gte=starting_datetime)&Q(cleaning_member_user__start_at__lt=ending_datetime)&Q(cleaning_member_user__end_at__gt=ending_datetime))|Q(Q(cleaning_member_user__end_at__gt=starting_datetime)&Q(cleaning_member_user__end_at__lte=ending_datetime)&Q(cleaning_member_user__start_at__lt=starting_datetime))),then=1),default=0,output_field=IntegerField()))).filter(cleaning_contained=0)
		else:
			workers            =  UserProfile.objects.filter(is_active=True).filter(Q(Q(user_type='TEAMINCHARGE')|Q(user_type='CLEANER'))).prefetch_related('leave_staff').annotate(leave=Sum( Case( When( leave_staff__leave_date=workers_date,then=1),default=0,output_field=IntegerField())) ).prefetch_related(Prefetch('cleaning_member_user',queryset=CleaningTeamMember.objects.filter( Q( Q(is_active=True)&Q(Q(start_at__date__gte=workers_date_start)&Q(end_at__date__lte=workers_date_end)) )).select_related('team__order_scheduler__customer_address__area','team__order_scheduler__order__evaluation','team__order_scheduler__order_scheduler_book'),to_attr='cleaning_member_details'),Prefetch('followup_member',queryset=FollowUpTeamMember.objects.filter(Q( Q(is_active=True)&Q(Q(start_at__date__gte=workers_date_start)|Q(end_at__date__lte=workers_date_end)) )).select_related('team__followup_scheduler__customer_address__area'),to_attr='followup_member_details'))
