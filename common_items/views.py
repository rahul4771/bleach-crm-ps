...
class OrderDetails(IsAuthenticated,View):
	def get(self,request):
		...
		if search:
			evaluations = Evaluation.objects.filter(is_active=True).filter(Q(estimated_cost__gt=0)|Q(quatation_status__isnull=False)).exclude(booking_evaluation__is_active=True).select_related('customer').filter(Q(Q(customer__name__icontains=search)|Q(customer__mobile_number__icontains=search)|Q(evaluation_id__icontains=search))).order_by('-id').prefetch_related(Prefetch('evaluation_order',queryset=Order.objects.filter(is_active=True),to_attr='evaluationorder')).annotate(order_in_progress_count=Count(Case(When( evaluation_order__order_status='ORDER_IN_PROGRESS',then=1),output_field=IntegerField())),order_closed_count=Count(Case(When( evaluation_order__order_status='ORDER_CLOSED',then=1),output_field=IntegerField())),order_cancelled_count=Count(Case(When( evaluation_order__order_status='ORDER_CANCELLED',then=1),output_field=IntegerField())),order_cancellinprogress_count=Count(Case(When( evaluation_order__order_status='CANCEL_IN_PROGRESS',then=1),output_field=IntegerField())),approved_not_paid_count=Count(Case(When( Q( Q(quatation_status='APPROVED') & Q(Q(Q(payment_method='PREPAID')&~Q(evaluation_order__payment_status='COMPLETED'))|Q(Q(payment_method='BREAKDOWN')&Q(evaluation_order__preamount_paid=0))) ),then=1),output_field=IntegerField())))
			bookings = Evaluation.objects.filter(is_active=True,booking_evaluation__is_active=True,created__gte=datetime.strptime('01-12-2022','%d-%m-%Y').replace(tzinfo=utc)).select_related('customer').filter(Q(Q(customer__name__icontains=search)|Q(customer__mobile_number__icontains=search)|Q(evaluation_id__icontains=search))).order_by('-id').prefetch_related(Prefetch('evaluation_order',queryset=Order.objects.filter(is_active=True),to_attr='evaluationorder')).annotate(order_in_progress_count=Count(Case(When( evaluation_order__order_status='ORDER_IN_PROGRESS',then=1),output_field=IntegerField())),order_closed_count=Count(Case(When( evaluation_order__order_status='ORDER_CLOSED',then=1),output_field=IntegerField())),order_cancelled_count=Count(Case(When( evaluation_order__order_status='ORDER_CANCELLED',then=1),output_field=IntegerField())),order_cancellinprogress_count=Count(Case(When( evaluation_order__order_status='CANCEL_IN_PROGRESS',then=1),output_field=IntegerField())),approved_not_paid_count=Count(Case(When( Q( Q(quatation_status='APPROVED') & Q(Q(Q(payment_method='PREPAID')&~Q(evaluation_order__payment_status='COMPLETED'))|Q(Q(payment_method='BREAKDOWN')&Q(evaluation_order__preamount_paid=0))) ),then=1),output_field=IntegerField())))
		else:
			evaluations = Evaluation.objects.filter(is_active=True).filter(Q(estimated_cost__gt=0)|Q(quatation_status__isnull=False)).exclude(booking_evaluation__is_active=True).select_related('customer').order_by('-id').prefetch_related(Prefetch('evaluation_order',queryset=Order.objects.filter(is_active=True),to_attr='evaluationorder')).annotate(order_in_progress_count=Count(Case(When( evaluation_order__order_status='ORDER_IN_PROGRESS',then=1),output_field=IntegerField())),order_closed_count=Count(Case(When( evaluation_order__order_status='ORDER_CLOSED',then=1),output_field=IntegerField())),order_cancelled_count=Count(Case(When( evaluation_order__order_status='ORDER_CANCELLED',then=1),output_field=IntegerField())),order_cancellinprogress_count=Count(Case(When( evaluation_order__order_status='CANCEL_IN_PROGRESS',then=1),output_field=IntegerField())),approved_not_paid_count=Count(Case(When( Q( Q(quatation_status='APPROVED') & Q(Q(Q(payment_method='PREPAID')&~Q(evaluation_order__payment_status='COMPLETED'))|Q(Q(payment_method='BREAKDOWN')&Q(evaluation_order__preamount_paid=0))) ),then=1),output_field=IntegerField())))
			bookings = Evaluation.objects.filter(is_active=True,booking_evaluation__is_active=True,created__gte=datetime.strptime('01-12-2022','%d-%m-%Y').replace(tzinfo=utc)).select_related('customer').order_by('-id').prefetch_related(Prefetch('evaluation_order',queryset=Order.objects.filter(is_active=True),to_attr='evaluationorder')).annotate(order_in_progress_count=Count(Case(When( evaluation_order__order_status='ORDER_IN_PROGRESS',then=1),output_field=IntegerField())),order_closed_count=Count(Case(When( evaluation_order__order_status='ORDER_CLOSED',then=1),output_field=IntegerField())),order_cancelled_count=Count(Case(When( evaluation_order__order_status='ORDER_CANCELLED',then=1),output_field=IntegerField())),order_cancellinprogress_count=Count(Case(When( evaluation_order__order_status='CANCEL_IN_PROGRESS',then=1),output_field=IntegerField())),approved_not_paid_count=Count(Case(When( Q( Q(quatation_status='APPROVED') & Q(Q(Q(payment_method='PREPAID')&~Q(evaluation_order__payment_status='COMPLETED'))|Q(Q(payment_method='BREAKDOWN')&Q(evaluation_order__preamount_paid=0))) ),then=1),output_field=IntegerField())))
			
		# booked_cleanings = CustomerBooking.objects.filter(is_active=True,booking_type='CLEANINGBOOKING').values_list('evaluation__id',flat=True)
		# bookings = Evaluation.objects.filter(id__in=booked_cleanings,is_active=True).prefetch_related(Prefetch('evaluation_order',queryset=Order.objects.filter(is_active=True),to_attr='evaluationorder')).annotate(order_in_progress_count=Count(Case(When( evaluation_order__order_status='ORDER_IN_PROGRESS',then=1),output_field=IntegerField())),order_closed_count=Count(Case(When( evaluation_order__order_status='ORDER_CLOSED',then=1),output_field=IntegerField())),order_cancelled_count=Count(Case(When( evaluation_order__order_status='ORDER_CANCELLED',then=1),output_field=IntegerField())),order_cancellinprogress_count=Count(Case(When( evaluation_order__order_status='CANCEL_IN_PROGRESS',then=1),output_field=IntegerField())),approved_not_paid_count=Count(Case(When( Q( Q(quatation_status='APPROVED') & Q(Q(Q(payment_method='PREPAID')&~Q(evaluation_order__payment_status='COMPLETED'))|Q(Q(payment_method='BREAKDOWN')&Q(evaluation_order__preamount_paid=0))) ),then=1),output_field=IntegerField())))
...
class ClientOrderDetails(IsAuthenticated,View):
	def get(self,request,order_id):
		...
		schedules = OrderScheduler.objects.filter(order__id=order_id,is_active=True)
		total_visit_count = schedules.count()
		completed_and_cancelled_count = schedules.filter(Q(work_status='CLEANING_FULFILLED')|Q(work_status='CLEANING_CANCELLED')).count()
		...
		return render(request,"common/client/order-page.html",{"invoice":invoice,"booking_id":booking_id,"order":order,"client_details":client_details,"active_orders_count":active_orders_count,"total_orders_count":total_orders_count,"average_feedback":average_feedback,"price_ranges":price_ranges,"total_visits":total_visit_count,"completed_cancelled_visits":completed_and_cancelled_count})
	...
	def get(self,request):
		...
		for date in daterange:
			...
			if date == todate:
				...
				daily_sales = round(net_amount)

			monthly_sales += float(net_amount)
...
class MakeQuatationPhase1(IsAuthenticated,View):
    ...
    def get(self, request, enquiry_id, evaluation_id):
        enquiry_user = UserProfile.objects.prefetch_related(
            Prefetch(
                'address_customer',
                queryset=Address.objects.filter(is_active=True).select_related('area', 'governorate'),
                to_attr='customer_addresses'
            )
        ).get(id=enquiry_id)

        evaluation = Evaluation.objects.filter(id=evaluation_id).prefetch_related(
            Prefetch(
                'booking_evaluation',
                queryset=CustomerBooking.objects.filter(is_active=True),
                to_attr='bookings'
            )
        ).first()

        evaluation_details = EvaluationDetails.objects.filter(
            is_active=True, evaluation=evaluation
        ).prefetch_related(
            Prefetch(
                'evaluation_book_evaluation_details',
                queryset=EvaluationBook.objects.filter(is_active=True, cleaning_policy='SUBSCRIPTION'),
                to_attr='evaluationbooks'
            )
        )

        evaluation_details_count = evaluation_details.count()
        evaluation_details_completed_count = evaluation_details.filter(status='EVALUATED').count()
        allow_submit = evaluation_details_count == evaluation_details_completed_count

        return render(request, 'common/enquiry/phase1quatation.html', {
    ...
    def post(self, request, enquiry_id, evaluation_id):
        ...
        if action == 'cancel':
            ...
            EvaluationDetails.objects.filter(id=evaluation_detail_id).update(
                status='CANCELLED',
                evaluation_cancel_reason=cancel_reason
            )
        ...
        Evaluation.objects.filter(id=evaluation_id, is_active=True).update(**evaluation_data)

        is_advance = request.POST.get('is_advance')
        order_update_data = {
            "total_amount": evaluation_data["total_cost"],
            "remining_amount": evaluation_data["total_cost"],
        }
        if is_advance:
        ...
        evaluation = order.evaluation
        evaluation_details = EvaluationDetails.objects.filter(evaluation=evaluation).select_related(
            'evaluator', 'address'
        ).first()

        evaluator = evaluation_details.evaluator.name if evaluation_details.evaluator else evaluation.call_attender.name

        evaluation_books = EvaluationBook.objects.filter(evaluation_details=evaluation_details).prefetch_related(
            Prefetch(
                'evaluationsection_book',
                queryset=EvaluationBookSection.objects.filter(is_active=True).prefetch_related(
                    Prefetch('addonsections', queryset=EvaluationSectionAddons.objects.filter(is_active=True), to_attr='sectionaddons')
                ),
                to_attr='sections'
            )
        )
        ...
        if evaluation.customer.is_sms:
            ...
            if evaluation.customerbooking:
                sms_url = f"https://my.bleachkw.com/customer/booking/invoice/paw{evaluation.evaluation_id[3:14]}{evaluation.customer.username}"
            else:
                sms_url = (
                    f"https://my.bleachkw.com/customer/subscription/quatation/paw{evaluation.tracking_no}{evaluation.customer.username}"
                    if evaluation.payment_method == 'SUBSCRIPTION'
            ...
            address_list = [
                value for value in [
                    evaluation_details.address.apartment,
                    evaluation_details.address.floor,
                    evaluation_details.address.street,
                    evaluation_details.address.building,
                    evaluation_details.address.avenue,
                    evaluation_details.address.block,
                    evaluation_details.address.area.name,
                    evaluation_details.address.governorate.name
                ] if value is not None
            ]
            separator = ", "

            message = (
                f"Dear Customer, Please find the Quotation against the cleaning at {separator.join(address_list)} here {sms_url}. "
                "For any assistance please contact us on +9651882707. Thank you for choosing Bleach Kuwait."
            ) if evaluation.customer.sms_preference == "ENGLISH" else (
                f"عزيزنا العميل نرجوا الاطلاع على عرض سعر خدمات التنظيف المطلوبة في {separator.join(address_list)} {sms_url} "
        ...
        if evaluation.customer.is_email:
            price_ranges = ServicePriceRange.objects.filter(is_active=True)
            email_template = "email/quatation.html" if not evaluation.customerbooking else "email/invoice.html"

            msg_html = render_to_string(email_template, {
                "evaluator": evaluator,
                "evaluation": evaluation,
                "evaluationbooks": evaluation_books,
                "address_list": separator.join(address_list),
        ...
        user_redirect_map = {
            ...
            'BOOKING_OFFICER': 'booking-officer:bookingofficerdash-board'
        }
...
class AssigncleaningTeam(IsAuthenticated,View):
	def get(self,request,scheduler_id):
		...
		shift_cleaners      = ShiftSchedule.objects.select_related('staff').filter(Q(Q(shift_date=start_at_date)|Q(shift_date=end_at_date)|Q(Q(shift3_start_at__lte=end_at_datetime)&Q(shift3_end_at__gte=end_at_datetime)))).filter(Q(Q(staff__user_type='CLEANER')|Q(staff__user_type='TEAMINCHARGE'))).filter(Q(Q(Q(shift1_start_at__lte=start_at_time)&Q(shift1_end_at__gte=start_at_time))&Q(Q(shift1_start_at__lte=end_at_time)&Q(shift1_end_at__gte=end_at_time))) | Q(Q(Q(shift2_start_at__lte=start_at_time)&Q(shift2_end_at__gte=start_at_time))&Q(Q(shift2_start_at__lte=end_at_time)&Q(shift2_end_at__gte=end_at_time))) | Q(Q(Q(shift3_start_at__lte=start_at_datetime)&Q(shift3_end_at__gte=start_at_datetime))&Q(Q(shift3_start_at__lte=end_at_datetime)&Q(shift3_end_at__gte=end_at_datetime)))).values_list('staff',flat=True)
		shift_leaders       = ShiftSchedule.objects.select_related('staff').filter(Q(Q(shift_date=start_at_date)|Q(shift_date=end_at_date)|Q(Q(shift3_start_at__lte=end_at_datetime)&Q(shift3_end_at__gte=end_at_datetime)))).filter(staff__user_type='TEAMINCHARGE').filter(Q(Q(Q(shift1_start_at__lte=start_at_time)&Q(shift1_end_at__gte=start_at_time))&Q(Q(shift1_start_at__lte=end_at_time)&Q(shift1_end_at__gte=end_at_time))) | Q(Q(Q(shift2_start_at__lte=start_at_time)&Q(shift2_end_at__gte=start_at_time))&Q(Q(shift2_start_at__lte=end_at_time)&Q(shift2_end_at__gte=end_at_time))) | Q(Q(Q(shift3_start_at__lte=start_at_datetime)&Q(shift3_end_at__gte=start_at_datetime))&Q(Q(shift3_start_at__lte=end_at_datetime)&Q(shift3_end_at__gte=end_at_datetime)))).values_list('staff',flat=True)
		...
		#excluding absent cleaners/active cleaners and including shift cleaners(8 -22 leave logic applied)
		leavestart_at_datetime1  = start_at_datetime.replace(hour=5,minute=0,second=0,microsecond=0)
		leaveend_at_datetime1    = start_at_datetime.replace(hour=19,minute=0,second=0,microsecond=0)
		...
		try:
			last_cleaning_team  = CleaningTeam.objects.select_related('order_scheduler__order','team_leader').filter(order_scheduler__order=order_schedule.order,end_at__lte=end_at_datetime).order_by('start_at').prefetch_related(Prefetch('cleaning_member_team',queryset=CleaningTeamMember.objects.select_related('member').filter(is_active=True,is_backup_cleaner=False),to_attr='cleaningteam_members')).last()
		except:
			last_cleaning_team  = None

		price_ranges 			= ServicePriceRange.objects.filter(is_active=True)

		return render(request,'common/cleaning/cleaningteam_assign.html',{'order_schedule':order_schedule,'order_schedules':order_schedules,'cleaners':cleaners,'leaders':leaders,'last_cleaning_team':last_cleaning_team,'price_ranges':price_ranges})
...
class EditcleaningTeam(IsAuthenticated,View):
	def get(self,request,scheduler_id):
		...
		#same blc cleaners for excluding
		sameblc_cleaners  = CleaningTeamMember.objects.select_related('team__order_scheduler__evaluation_details__evaluation').filter(team__order_scheduler__evaluation_details__evaluation=order_schedule.evaluation_details.evaluation,is_backup_cleaner=False).filter(Q(Q(Q(start_at__gte=start_at_datetime)&Q(start_at__lte=end_at_datetime))|Q(Q(end_at__gte=start_at_datetime)&Q(end_at__lte=end_at_datetime))|Q(Q(start_at__lte=start_at_datetime)&Q(end_at__gte=start_at_datetime)&Q(start_at__lte=end_at_datetime)&Q(end_at__gte=end_at_datetime))|Q(Q(start_at__gte=start_at_datetime)&Q(end_at__gte=start_at_datetime)&Q(start_at__lte=end_at_datetime)&Q(end_at__lte=end_at_datetime)))).values_list("member",flat=True)
	...
	def post(self,request,scheduler_id):
		...
		if check_cleaners.count() >= len(assigned_cleaners) and check_tl:
			for order_schedule in order_schedules:
				...
				cleaners_to_be_deleted       = CleaningTeamMember.objects.filter(team__order_scheduler=order_schedule,is_backup_cleaner=False).delete()
...
class CashCollect(IsAuthenticated,View):
	def get(self,request):
		return render(request,'common/payment/cash-collect.html',{})
	def post(self,request):
		...
		return redirect('common_items:cashcollect')


class FineWriteBack(IsAuthenticated,View):
	def get(self,request):
		return render(request,'common/payment/finewriteback.html',{})
	def post(self,request):
		order_id  = request.POST.get('order_id')
		order     = Order.objects.get(id=order_id)
