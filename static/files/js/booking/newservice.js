$(document).ready(function(){
  ...
  const app=new Vue({
      ...
      data: {
        ...
        subStat:'',
        altdaysStat:false,
        currentPageTitle:'Add / Remove Services',
        custBookStat:false,
        scheduleStat:true,
        serviceChange:true,
          cat_counter:0,
    ...
    selectPayment(pay){
     ...
     getFiles(obj){
      ...
      arrangeData(){
            for(var i=0;i<this.multiServicesBill.length;i++){
              ...
              for(var j=0;j<this.multiServicesBill[i].bill.length;j++){
                ...
                if(this.multiServicesBill[i].bill[j].section.color){
                    if(Array.isArray(this.multiServicesBill[i].bill[j].section.color))
                    {
                ...
                if(this.multiServicesBill[i].bill[j].section.material){
                    if(Array.isArray(this.multiServicesBill[i].bill[j].section.material))
                    {
             ...
             {
                if(Array.isArray(this.multiServicesBill[i].bill[j].section.wall_type))
                {
               this.serviceDetails.service_details[i].sections[j].wall_type=this.multiServicesBill[i].bill[j].section.wall_type.join()
                }
                else{
                    this.serviceDetails.service_details[i].sections[j].wall_type=this.multiServicesBill[i].bill[j].section.wall_type
                }
             }
             if(this.multiServicesBill[i].bill[j].section.ceiling_type)
             {
                if(Array.isArray(this.multiServicesBill[i].bill[j].section.ceiling_type))
                {
                    this.serviceDetails.service_details[i].sections[j].ceiling_type=this.multiServicesBill[i].bill[j].section.ceiling_type.join()
                }
                else{
                    this.serviceDetails.service_details[i].sections[j].ceiling_type=this.multiServicesBill[i].bill[j].section.ceiling_type
                }
             ...
             {
                if(Array.isArray(this.multiServicesBill[i].bill[j].section.floor_type))
                {
                    this.serviceDetails.service_details[i].sections[j].floor_type=this.multiServicesBill[i].bill[j].section.floor_type.join()
                }
                else{
                    this.serviceDetails.service_details[i].sections[j].floor_type=this.multiServicesBill[i].bill[j].section.floor_type
                }
           ...
           if(this.multiServicesBill[i].bill[j].section.keynote_data)
           {
  ...
  responsive:{
    ...
    bookMultipleService(){
     ...
     var posturl=''
     this.serviceDetails['id']=params.eval_id
     if(this.scheduleStatus){
       posturl='/customer/service/add-delete/'
     
      axios
        .post(
           this.url+posturl+params.eval_id,this.serviceDetails
  ...
  */
           ...
           // var total_curtain_size = 750;
            ...
            var manhour =
              ...
              for(var ao=0;ao<this.multiServicesBill[this.schedule_serviceTypes_selected[k]].bill.length;ao++){
                  for(var addon=0;addon<this.multiServicesBill[this.schedule_serviceTypes_selected[k]].bill[ao].section.addonsections.length;addon++){
                    if(this.multiServicesBill[this.schedule_serviceTypes_selected[k]].bill[ao].section.addonsections[addon].selected){
                      if(this.multiServicesBill[this.schedule_serviceTypes_selected[k]].bill[ao].section.addonsections[addon].details.category){
                        addon_manhour=addon_manhour+(this.multiServicesBill[this.schedule_serviceTypes_selected[k]].bill[ao].section.addonsections[addon].quantity*(parseInt(this.multiServicesBill[this.schedule_serviceTypes_selected[k]].bill[ao].section.addonsections[addon].selected_size.max_size)/this.multiServicesBill[this.schedule_serviceTypes_selected[k]].bill[ao].section.addonsections[addon].details.productivity))
                      }
                      else{
                      addon_manhour=addon_manhour+this.multiServicesBill[this.schedule_serviceTypes_selected[k]].bill[ao].section.addonsections[addon].details.productivity*this.multiServicesBill[this.schedule_serviceTypes_selected[k]].bill[ao].section.addonsections[addon].quantity
                      }
                    }
                  }
    ...
    for(var i=0;i<serviceBookedDetails.length;i++)
    {
      ...
      for(var j=0;j<serviceBookedDetails[i].evaluationsection_book.length;j++) {
        ...
        scheduleDetails.bill.push({
            ...
            section_name:serviceBookedDetails[i].evaluationsection_book[j].section_name,
            section_net_cost:serviceBookedDetails[i].evaluationsection_book[j].section_cost,
            sectiononly_cost:serviceBookedDetails[i].evaluationsection_book[j].sectiononly_cost,
            sectiononly_net_cost:serviceBookedDetails[i].evaluationsection_book[j].sectiononly_cost
        })
      }
  ...
  async rearrangeSize(){
    ...
    for(var i=0;i<this.multiServicesBill.length;i++){
      ...
      for(var j=0;j<this.multiServicesBill[i].bill.length;j++){
        /* If old bookings */
        if(!isNaN(this.multiServicesBill[i].bill[j].section.size)){
        
          for(var p in productivity){
             /** for kitchen Cleaning*/
             if(this.multiServicesBill[i].service=='Kitchen Cleaning'){
                if(multiServicesBill[i].bill[j].section.new_kitchen){
                  if(productivity[p].name==this.multiServicesBill[i].bill[j].section.size && productivity[p].is_newkitchen && parseInt(this.multiServicesBill[i].bill[j].section.size)>=productivity[p].min_size && parseInt(this.multiServicesBill[i].bill[j].section.size)<=productivity[p].max_size){
        ...
        /**  for general,deep,carparking,sterilization....... */
            ...
            if((parseInt(this.multiServicesBill[i].bill[j].section.size)>=productivity[p].min_size) && (parseInt(this.multiServicesBill[i].bill[j].section.size)<=productivity[p].max_size)){
        ...
        if(this.multiServicesBill[i].service=='Kitchen Cleaning'){
          ...
          if(this.multiServicesBill[i].bill[j].section.new_kitchen)
          {
            for(var p in productivity){
          if(productivity[p].name==this.multiServicesBill[i].bill[j].section.size && productivity[p].is_newkitchen){
            this.multiServicesBill[i].bill[j].section.size=productivity[p]
          }
        }
        }
          ...
          console.log("up type is "+this.multiServicesBill[i].bill[j].section.upholstery_type)
          var type=""
           for(var j=0;j<this.multiServicesBill[i].bill.length;j++){
             if(this.multiServicesBill[i].bill[j].section.upholstery_type=='SOFA'){
             
               if(this.multiServicesBill[i].bill[j].section.size.includes('Seater')){
                this.multiServicesBill[i].bill[j].section.size=this.multiServicesBill[i].bill[j].section.size.split(" ")[0]
                var seater_size=parseInt(this.multiServicesBill[i].bill[j].section.size)
                for(var p in productivity){
                    if(seater_size<=productivity[p].max_size && seater_size>=productivity[p].min_size){
                        this.multiServicesBill[i].bill[j].section.size={
                            combinedSize:seater_size+' Seater',
                            name:seater_size+' Seater',
                            max_size:seater_size,
                            min_size:seater_size,
                            upholstery_type:this.multiServicesBill[i].bill[j].upholstery_type,
                            cost:productivity[p].cost
                        }
                    }
                    else{
                        this.multiServicesBill[i].bill[j].section.size={
                            ...
                            min_size:seater_size,
                            upholstery_type:this.multiServicesBill[i].bill[j].upholstery_type,
                            cost:productivity[p].unit_price*seater_size
                        }

                    }
                    if(productivity[p].name==this.multiServicesBill[i].bill[j].section.size && productivity[p].upholstery_type=='SOFA'){
                      this.multiServicesBill[i].bill[j].section.size=productivity[p]
                    }
          ...
          for(var j=0;j<this.multiServicesBill[i].bill.length;j++){
          for(var p in productivity){
          if(this.multiServicesBill[i].bill[j].section.is_highprice_facade){
            if(productivity[p].name==this.multiServicesBill[i].bill[j].section.size && productivity[p].is_highprice_facade ){
              this.multiServicesBill[i].bill[j].section.size=productivity[p]
            }
          ...
          for(var j=0;j<this.multiServicesBill[i].bill.length;j++){
          for(var p in productivity){
          if(this.multiServicesBill[i].bill[j].section.is_highprice_window){
            if(productivity[p].name==this.multiServicesBill[i].bill[j].section.size && productivity[p].is_highprice_window ){
              this.multiServicesBill[i].bill[j].section.size=productivity[p]
            }
        ...
        for(var p in productivity){
            ...
            if(productivity[p].name==this.multiServicesBill[i].bill[j].section.size){
              this.multiServicesBill[i].bill[j].section.size=productivity[p]
            }
  ...
  async rearrangeKitchenAddons(){
      for(var i=0;i<this.multiServicesBill.length;i++){
          for(var j=0;j<this.multiServicesBill[i].bill.length;j++){
            this.multiServicesBill[i].bill[j].section['kitchens']=[]
            this.multiServicesBill[i].bill[j].section['kitchen']=false
              for(var k=0;k<this.multiServicesBill[i].bill[j].section.addonsections.length;k++){
                  if(this.multiServicesBill[i].bill[j].section.addonsections[k].name=='kitchen'){
                    this.multiServicesBill[i].bill[j].section['kitchen']=true
                    var kitchendata=JSON.parse(this.multiServicesBill[i].bill[j].section.addonsections[k].other_details)
                    var conv_size=await this.getConvKitchenSize(kitchendata)
                    console.log("conv size s"+JSON.stringify(conv_size))
                    this.multiServicesBill[i].bill[j].section.kitchens.push({
                        ceiling_type:"",
                        condition:"",
                        floor_type:"",
                        is_cabinet:kitchendata.is_cabinet,
                        residue:kitchendata.residue,
                        size:conv_size,
  ...
  async getConvKitchenSize(kitchendata){
      for(var i=0;i<this.kitchenSizeData.length;i++){
          if(kitchendata.type=='old'){
              if(kitchendata.is_cabinet){
                  if(!this.kitchenSizeData[i].is_newkitchen && this.kitchenSizeData[i].is_cabinet){
                      return this.kitchenSizeData[i]
                  }
              }
              else{
                if(!this.kitchenSizeData[i].is_newkitchen && !this.kitchenSizeData[i].is_cabinet){
