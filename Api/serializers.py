...
class ShiftScheduleSerializer(serializers.ModelSerializer):
    shift3_start_at = serializers.DateTimeField(input_formats=["%d-%m-%Y %I:%M %p"],required=False)
    shift3_end_at   = serializers.DateTimeField(input_formats=["%d-%m-%Y %I:%M %p"],required=False)
    class Meta:
        model = ShiftSchedule
