...
class InvestigationForm(forms.ModelForm):
	...
	def __init__(self,*args,**kwargs):
		...
		self.fields['investigator'] = forms.ModelChoiceField(
			queryset=UserProfile.objects.filter(Q(Q(user_type='QUALITYCONTROLL')|Q(user_type='OPERATIONSUPERVISOR')|Q(user_type='SENIORTEAMLEADER')),is_active=True),required=True,widget=forms.Select(attrs={'class':'userprofile'}))
