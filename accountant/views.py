...
class ActiveSubscriptions(IsAccountant,View):
	def get(self,request):
		...
		if subscriptions:
			completed_cleanings_count = subscriptions.completed_cleanings_count
			subscriptions.exclude(Q( Q(total_cleaning_count = completed_cleanings_count) & Q(payment_status='COMPLETED') ))
